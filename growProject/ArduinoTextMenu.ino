...
int dzien = 1;int miesiac = 1;int rok = 2022;int sekundy = 0;int minuty = 0;int godziny = 0; // default value
...
String dayAsString(const Time::Day day) {
  switch (day) {
    case Time::kSunday: return "Niedziela";
    case Time::kMonday: return "Ponidzialek";
    case Time::kTuesday: return "Wtorek";
    case Time::kWednesday: return "Sroda";
    case Time::kThursday: return "Czwartek";
    case Time::kFriday: return "Piatek";
...
enum eMenuKey {mkNull, mkBack, mkRoot, mkMenu1, mkSettings, mkMenu2, mkEnterDzien,mkEnterMiesiac,mkEnterRok, mkEnterSec,mkEnterMin,mkEnterHour, mkMenu3,mkDHT11, mkMenu4,mkCapacity, mkMenu5,mkGroundTemp,mkMenu6,mkLightMeter, mkMenu7,mkRelay, mkMenu8, mkAutomatManual
              };
...
void AutomaticManual(){ 
  ...
  while (1)
  {
    ...
    switch (EncoderState) {
      ...
      case eButton: {
          LCDBacklight(1);
          if(choiseMenu == 0 && modeChoise == false){modeChoise = true; break;};
          if(choiseMenu == 0 && modeChoise == true){modeChoise = false; break;};
          if(autoManual == false){ //manual mode
            if(choiseMenu == 1 && periodChoise == false){periodChoise = true; break;};
            if(choiseMenu == 1 && periodChoise == true){periodChoise = false; break;}; // zapisat izmeneni9 period
            if(choiseMenu == 2 && timerChoise == false){timerChoise = true; break;};
            if(choiseMenu == 2 && timerChoise == true){timerChoise = false; break;}; // zapisat izmeneni9 time
            //}else{
              if(choiseMenu == 3){
                return;}; //=========================================================?
              };
          if(autoManual && choiseMenu){return;};
      ...
      case eLeft: {
        lcd.clear();
          LCDBacklight(1);
          if(modeChoise)autoManual = false;
          if(!modeChoise && !periodChoise && !timerChoise){ // esli net modow dla izmenenii
                if(choiseMenu <=0)choiseMenu=0;
              else 
              choiseMenu--;
                }else{
                  // proverka kogo izmenit nuzno
                    if(periodChoise == true){
                      if(periodTime <1) periodTime=255;else
                      periodTime--;
                    };
                    if(timerChoise == true){
                      if(waterOnTime <1) waterOnTime=255;else
                      waterOnTime--;
                    };
                  };
      ...
      case eRight: {
        lcd.clear();
          LCDBacklight(1);
          if(modeChoise)autoManual = true;
          if(!modeChoise && !periodChoise && !timerChoise){


            if(autoManual){
              if(choiseMenu >=1)choiseMenu=1;
              else 
              choiseMenu++;
            }else{
              if(choiseMenu >=3)choiseMenu=3;
              else 
              choiseMenu++;
              }
            ...
            //else{
                    if(periodChoise == true){                       // +- 1 hour, byte ? 0-255
                      if(periodTime >254) periodTime=1;else
                      periodTime++;
                    };
                    if(timerChoise == true){                        // +- 1 sec, byte ? 0-255
                      if(waterOnTime >254) waterOnTime=1;else
                      waterOnTime++;
                    };
                  //};
          break;
    ...
    writeAutoManMenu(autoManual);
    //arrowPosition(choiseMenu,autoManual);
    if(periodChoise || timerChoise){additionalDataWrite(&periodTime,&waterOnTime);}
    else{arrowPosition(choiseMenu,autoManual);}
    ;
  }
  };
...
void Frelay(){
  ...
  switch(choise){
    case 1:{
        lcd.setCursor(0,0);
        lcd.print("->");
        if(choiseFlag){offOnShow(choise, offOn);       
        };
        break;
        }
      case 2:{
...
void DS18B20(){
  ...
  if(millis() - myTimer1 > 2000){
    ...
    myTimer1 = millis();
    sensorDS18B20.requestTemp();
    if (sensorDS18B20.readTemp()) lcd.print(sensorDS18B20.getTemp());
...
// полить                             (data4)
  ...
  if(calibrParameters[0] != 0 && calibrParameters[1] != 0 && calibrParameters[2] != 0 && calibrParameters[3] != 0){ //если есть уже данные в глобальном массиве то выводим то что есть 
        for (int i = 0; i <= 3; i++) {
  ...
  {
    if(exitFrom == 4){
      if(parameters[0] != 0 && parameters[1] != 0 && parameters[2] != 0 && parameters[3] != 0){ //если выкрутили выход и все значения заданы то пишем в глобальный массив (ЗАПИСЬ В EEPROM ОРГАНИЗОВАТЬ ОТДЕЛЬНО)
          for (int i = 0; i <= 3; i++) {
